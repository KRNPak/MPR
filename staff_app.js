...
let selectedMonth = 'Jul'; // Defaults to start of year until parsed
let timeView = 'YTD'; 

let positionMaster = {}; 
let unifiedLedger = []; // Array of monthly records per position
let activeFilteredData = [];
let activeMonths = [];

let widgetView = 'vacant'; // 'vacant' or 'overspend'
...
// Map varying CSV columns to standard HR Components
function standardizeComponent(keyStr) {
    const k = keyStr.toLowerCase();
    if (k.includes('base')) return 'Base Salary';
    if (k.includes('car') || k.includes('cma')) return 'Car Monetization';
    if (k.includes('cola')) return 'COLA';
    if (k.includes('child')) return 'Child Care';
    if (k.includes('lfa') || k.includes('leave fare')) return 'LFA';
    if (k.includes('provident') || k.includes('pf')) return 'Provident Fund';
    if (k.includes('eobi')) return 'EOBI';
    if (k.includes('gratuity')) return 'Gratuity';
    if (k.includes('gross') || k.includes('total') || k.includes('net') || k.includes('month') || k.includes('date') || k.includes('position') || k.includes('employeename') || k.includes('name') || k.includes('department') || k.includes('designation')) return 'SKIP';
...
function buildDataEngine(masterRows, budgetRows, actualRows) {
    ...
    // 2. Build Unified Ledger
    const ledgerMap = {}; 
    const ensureLedgerEntry = (code, mth) => {
        let key = `${code}_${mth}`;
        if (!ledgerMap[key]) {
            ledgerMap[key] = { code, month: mth, components: {} };
        }
    ...
    actualRows.forEach(r => {
        ...
        mth = mth.charAt(0).toUpperCase() + mth.slice(1).toLowerCase(); // Normalize to 'Jul', 'Aug'
    ...
    // Parse Budget (Apply monthly run-rate based on Joining Date)
    budgetRows.forEach(r => {
        ...
        // Very basic month mapping for joining date
        let startIdx = 0; 
        ...
        Object.keys(r._raw).forEach(rawK => {
            ...
            if (clean.includes('updatedbaseannually') || clean.includes('updatedgrosssalaryannually')) return; // Ignore annual aggregations
            let compName = standardizeComponent(rawK);
            if (compName !== 'SKIP') {
                let val = getSafeNum(r._raw[rawK]);
                // Assume budget CSV values are Monthly rates based on screenshot. 
                // If they are annual, add logic: val = val / 12;
                monthlyComps[compName] = val; 
            }
        });
        
        // Also add base manually if it's strictly in 'updatedbaseannually'
        let annBase = getSafeNum(r['updatedbaseannually']);
        if (annBase > 0) monthlyComps['Base Salary'] = annBase / 12;

        // Apply to fiscal months based on active window
    ...
    // Set most recent actuals month as default
    let monthArr = Array.from(availableMonthsSet);
    if (monthArr.length > 0) {
        // Find latest month in fiscal order
        let latestIdx = -1;
        monthArr.forEach(m => {
            let idx = fiscalMonths.indexOf(m);
...
function updateStaffDashboard() {
    ...
    unifiedLedger.forEach(row => {
        ...
        // Donor Split Tracking (Use Actuals. If no actuals, fall back to Budget)
    ...
    renderDonorDonut(donorSpends, totAct || totBud); // Fallback to budget if no actuals
    renderWidget(positionSpends);
}

// ========================================================================
// 4. UI RENDERING (Table, Widget, Donut)
// ========================================================================
function renderComponentTable(compSummary) {
    const tbody = document.getElementById('componentTableBody');
    tbody.innerHTML = '';
    
    // Sort components (Base first, then size of actuals)
...
function setWidgetView(view) {
    ...
    updateStaffDashboard(); // Re-render lists
}
...
function renderWidget(positionSpends) {
    ...
    if (widgetView === 'vacant') {
        // Vacant = Budget > 0 but Actual = 0
        displayList = list.filter(p => p.b > 0 && p.a === 0).sort((x, y) => y.b - x.b);
        
        if (displayList.length === 0) {
            container.innerHTML = '<div style="padding:20px; text-align:center; color:var(--text-secondary); font-size:0.75rem;">No vacant positions in period.</div>';
            return;
        }
        ...
        // Overspend = Actual > Budget (Sort by largest negative variance)
        displayList = list.filter(p => p.a > p.b).sort((x, y) => (y.a - y.b) - (x.a - x.b));
        
        if (displayList.length === 0) {
            container.innerHTML = '<div style="padding:20px; text-align:center; color:var(--text-secondary); font-size:0.75rem;">No over-spends detected.</div>';
            return;
        }
...
function renderDonorDonut(donorSpends, totalVal) {
    if (Object.keys(donorSpends).length === 0) {
        document.getElementById('donorDonutContainer').innerHTML = '<div style="font-size:0.7rem; color:var(--text-secondary); text-align:center; margin-top:40px;">No Funding Data</div>';
        return;
    }
...
// ========================================================================
// 5. MODAL (Period Breakup Drill-Down)
// ========================================================================
...
function renderStaffModalTable() {
    ...
    // Build Headers Dynamically based on activeMonths
    ...
    // Aggregate by employee for the specific component
    let empData = {};
    ...
    Object.values(empData).sort((x, y) => y.tA - x.tA).forEach(p => {
        ...
        activeMonths.forEach(m => {
            rowHtml += `<td style="font-variant-numeric:tabular-nums">${(p.months[m]/1000000).toFixed(2)}</td>`;
        });
...
function formatPKRShort(num) {
    let p = getFormattedParts(num); return `${p.v} ${p.u.charAt(0)}`; 
}
    ...
    container.innerHTML = `
        <div class="svg-donut-wrapper">
            <div class="donut-chart-box">
                <svg viewBox="0 0 100 100" class="donut-svg">${circlesHtml}</svg>
            </div>
